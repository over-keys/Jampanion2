...
checks = {
    ...
    "bridge startup errors target the same origin": 'const bridgeTargetOrigin = window.location.origin === "null" ? "*" : window.location.origin' in viewer_source,
    ...
    "playback uses one canonical expansion": 'function buildCanonicalPlaybackSequence' in host and host.count('    const expanded = buildCanonicalPlaybackSequence(loopSource, sourceOffset, expandChartBars);') == 2,
    ...
    "background playback protects prequeued audio": 'BACKGROUND_LOOK_AHEAD_SECONDS = 4.0' in audio_source and 'getProtectedThrough' in audio_source and 'GetProtectedThroughAsync' in logic,
    ...
    "mobile controls are separated from the mix": 'jamp-mobile-controls' in home and 'position:fixed; top:0; left:0; right:0; z-index:30; order:1;' in (root / "src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text() and '.mix-panel { order:4;' in (root / "src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text(),
    ...
    "startup shell is standalone": 'Jampanion2' in index and '<strong>Jampanion</strong>' not in index and 'const APP_VERSION = \"36\"' in index,
    "cache-busted page versions": 'APP_VERSION = \"36\"' in index and 'help.html?v=36' in viewer_source and 'help.en.html?v=36' in viewer_source and 'help.css?v=36' in help_source and 'jazz-chart-host.js?v=37' in viewer_source and 'jazz-chart-host.js?v=37' in logic and 'viewer/index.html?integrated=37' in home,
    "standalone viewer keeps the accompaniment link": 'id="jampanionModeLink"' in viewer_source and 'href="../"' in viewer_source and 'removeAttribute("hidden")' in viewer_source and 'jampanion-32.png?v=37' in viewer_source,
    "viewer has offline install metadata": 'manifest.webmanifest?v=1' in viewer_source and 'viewer-service-worker.js' in viewer_source and 'serviceWorker.register' in viewer_source,
    "viewer standalone skips embedded bridge": 'window.parent === window' in viewer_source and 'new URL(location.href).searchParams.has("integrated")' in viewer_source,
    ...
    "viewer toolbar uses Delete for new songs and Revert for imports": 'jampanionStandaloneRevert' in host and 'toolbarDeleteVisible' in host and 'standaloneRevertButton.textContent = toolbarDeleteVisible ? "Delete" : "Revert"' in host and 'requestEmbedded("setToolbarState", { dirty, canRevert, canDelete }' in host and 'setToolbarState(args.dirty, args.canRevert, args.canDelete)' in host and 'DeleteNativeSongFromToolbar' in logic and 'CanDeleteCurrentNativeSong' in logic and 'CurrentSongHasSavedOverrides' in logic,
    ...
    "playback errors survive rollback notifications": 'isPlaybackRollback' in logic and '_playbackErrorIdentity' in logic and 'SetPlaybackError' in logic,
    ...
    "mobile CSS cache is bumped": 'jazz-integration.css?v=36' in home,
    ...
    "mobile control order and conditional fade": 'session-panel { order:1' in (root / "src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text() and 'label + label { order:3' in (root / "src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text() and 'can-scroll-left' in host and 'can-scroll-right' in host,
    "mobile session frame and field captions are removed": 'border:0; border-radius:0; background:transparent; padding:0;' in (root / "src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text() and '.compact-fields span { display:none; }' in (root / "src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text() and 'aria-label="Tempo"' in home and 'aria-label="Style"' in home,
    "mobile Mix fits inside the viewport": 'height:58px; max-height:58px; min-height:0; overflow:hidden;' in (root / "src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text(),
    "narrow mobile Mix keeps all channels visible": '@media (max-width:350px)' in (root / "src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text() and 'repeat(4,minmax(56px,1fr))' in (root / "src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text(),
    "mobile browser chrome can collapse without forced spacer": 'html, body { height:auto; min-height:100%; overflow-x:hidden; overflow-y:auto; }' in (root / "src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text() and '#app { height:auto; min-height:100dvh; overflow:visible; }' in (root / "src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text() and '.app-shell { height:auto; min-height:100svh; overflow:visible; display:block; }' in (root / "src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text(),
    "mobile controls align and match chart row": 'padding:3px 6px' in (root / "src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text() and 'height:30px; min-height:30px' in (root / "src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text() and '.jamp-mobile-controls .tiny { border-radius:5px; }' in (root / "src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text() and 'session-controls button { height:30px!important; min-height:30px!important; max-height:30px;' in (root / "src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text() and '.session-panel { width:166px; height:50px; }' in (root / "src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text() and '.jamp-brand-row { order:4; align-self:center; flex:0 0 36px; width:max-content; height:36px;' in (root / "src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text() and '.jamp-brand-row strong { display:block; font-size:12px; }' in (root / "src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text() and 'initializeMobileControlsScrollHint' in logic,
    "mobile brand is below Mix": home.find('<section class="jamp-panel mix-panel">') < home.find('<section class="jamp-brand-row">'),
    "mobile playback keeps controls available": '.jamp-shell.is-playing .jamp-brand-row { display:none; }' not in (root / "src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text() and '.jamp-shell.is-playing .accompaniment-panel { display:none; }' not in (root / "src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text() and '.jamp-shell.is-playing .jamp-mobile-controls { overflow:hidden; }' not in (root / "src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text(),
    ...
    "visible footer status is omitted": 'jamp-footer-status' not in home and 'jamp-footer-status' not in (root / "src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text(),
    ...
    "Viewer selection does not clear transpose": 'JAMPANION_SELECTION_TRANSPOSE_V1' in viewer_source and 'function selectSong(songId) {' in viewer_source and 'state.searchOpen = false;' in viewer_source and 'state.semitones = 0;' not in viewer_source[viewer_source.find('function selectSong(songId) {'):viewer_source.find('function selectSong(songId) {') + 400],
    "Revert asks whether changes should be reverted": 'window.confirm("Revert changes?")' in host and 'export async function revertCurrentSong()' in host,
    ...
    "mode switching uses matching-weight Viewer Mode link": 'jamp-mode-row' in home and '>Viewer Mode<' in home and 'jampanion-viewer.png' in home and '.jamp-mode-row img { width:22px; height:22px;' in css and 'font-size:15px; font-weight:400;' in css and '.jamp-mode-row a { font-size:12px; font-weight:400; }' in css and 'installStandaloneModeLink' in host and 'jampanion-viewer.png' in viewer_source and 'jampanion-mode-link' in host,
    "embedded startup waits for saved song": 'viewer.libraryReady' in host and 'await loadNativeSongs()' in host and 'jampanion-startup-pending' in host and 'libraryReady' in (root / "scripts/test-viewer-contract.mjs").read_text() and 'Loading saved song' in viewer_source and 'using demo fallback' in viewer_source,
    "delete-all clears Jampanion restore state": 'jampanion-library-cleared' in viewer_source and 'handleLibraryCleared' in host and 'removeSongSettings(identity)' in host and 'localStorage.removeItem(LAST_SONG_KEY)' in host,
    ...
    "customized songs restore original imports": 'annotateSongOptions' in host and 'jampanion-customized-marker' in host and 'updateCustomizedSearchTitle' in host and 'deleteCustomizedSongs' in host and 'restoreSongsByIds' in host and 'will be kept' in host and 'restoreSongsByIds' in viewer_source,
    "demo edits are treated as customized imports": 'function isDemoSong(song)' in host and 'originalSourceSong' in host and 'if (originalSong)' in host and 'hasOriginalSource: hasOriginalChart(song)' in host,
    ...
    "background playback does not stop on page visibility": 'visibilityHandler' not in host and 'StopSessionFromVisibility' not in host and 'jampanion-page-hidden' not in browser_source and 'jampanion-audio.js?v=36' in logic,
    "published shell version matches": '"version": "36"' in (root / "src/Jampanion.Web/wwwroot/app-version.json").read_text(),
    ...
    "viewer worker is preserved by root cleanup": 'scope.href === baseUrl.href' in index and 'baseUrl.href.startsWith(scope.href)' not in index and 'scope.href.startsWith(baseUrl.href)' not in index,
    ...
    "shell refresh query is cleared after boot": 'SHELL_REFRESH_QUERY' in index and 'clearBootQueries' in index,
    "continuation keeps the protected audio cursor": 'findCursorAt(scheduledThroughSeconds + 0.0001, true)' in audio_source,
