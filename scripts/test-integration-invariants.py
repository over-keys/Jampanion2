...
checks = {
    ...
    "mobile controls are separated from the mix": 'jamp-mobile-controls' in home and 'position:fixed; top:0; left:0; right:0; z-index:30; order:1;' in (root / "integration/overlay/src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text() and '.mix-panel { order:4;' in (root / "integration/overlay/src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text(),
    ...
    "mobile CSS cache is bumped": 'jazz-integration.css?v=27' in home,
    ...
    "mobile browser chrome can collapse without forced spacer": 'html, body { height:auto; min-height:100%; overflow-x:hidden; overflow-y:auto; }' in (root / "integration/overlay/src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text() and '#app { height:auto; min-height:100dvh; overflow:visible; }' in (root / "integration/overlay/src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text() and '.app-shell { height:auto; min-height:100svh; overflow:visible; display:block; }' in (root / "integration/overlay/src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text(),
