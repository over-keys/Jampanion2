...
checks = {
    ...
    "desktop brand row is at pane bottom": '.jamp-brand-row { order:99; align-self:center; margin-top:auto;' in (root / "integration/overlay/src/Jampanion.Web/wwwroot/css/jazz-integration.css").read_text(),
    ...
    "blank chord cells add at the clicked beat": 'A rendered chord slot can span several empty beat cells' in host and 'first?.classList.contains("cell-positioned-slot")' in host and 'const maxInputCells = meter === "3/4" ? 3 : 4' in host and 'Math.min(maxInputCells, Math.max(1, renderedTotal || maxInputCells))' in host and 'const existingIndex = sourceSlots.findIndex' in host,
