...
grep -q 'jampanion-audio.js?v=31' "$LOGIC"
...
grep -q 'jazz-chart-host.js?v=52' "$ROOT/scripts/customize-viewer.mjs"
grep -q 'jazz-chart-host.js?v=52' "$ROOT/integration/overlay/src/Jampanion.Web/Pages/IntegratedHomeLogic.cs"
...
grep -q 'bridgeOriginMatches(event.origin, frameOrigin)' "$HOST"
grep -q 'bridgeOriginMatches(event.origin, window.location.origin)' "$HOST"
grep -q 'BACKGROUND_LOOK_AHEAD_SECONDS = 4.0' "$ROOT/scripts/customize-background-playback.mjs"
grep -q 'getProtectedThrough' "$ROOT/scripts/customize-background-playback.mjs"
grep -q 'GetProtectedThroughAsync' "$LOGIC"
