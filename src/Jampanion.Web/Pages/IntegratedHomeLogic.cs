...
public class IntegratedHomeLogic : ComponentBase, IAsyncDisposable
{
    ...
    private async Task<bool> TryConnectChartAsync(bool reportFailure)
    {
        try
        {
            ...
            _chartModule ??= await JS.InvokeAsync<IJSObjectReference>("import", "./js/jazz-chart-host.js?v=38");
    ...
    private static int LiveReplacementSegmentLimit(
        JazzPlaybackFormDto chart,
        int boundarySequenceIndex)
    {
        // Live tempo/style changes only need enough newly arranged material to
        // reach the requested handoff plus a short safety buffer. Building all
        // 12 open-ended choruses here makes the UI miss the intended boundary
        // and needlessly loads the WebAssembly main thread while audio is live.
        const int lookAheadSegments = 3;
        var targetSequenceIndex = Math.Max(0, boundarySequenceIndex);
        var sequenceBase = 0;
        ...
        for (var chorus = 1; chorus <= IntegratedSessionPlanner.MaximumOpenEndedChoruses; chorus++)
        {
            var barCount = chorus == 1
                ...
                : chart.SoloBars.Count;
            if (barCount <= 0) break;

            if (targetSequenceIndex < sequenceBase + barCount)
            {
                var barsIntoStage = targetSequenceIndex - sequenceBase;
                var segmentsIntoStage =
                    barsIntoStage / SessionConstants.BarsPerSegment + 1;
                return Math.Max(
                    2,
                    segmentsBeforeStage + segmentsIntoStage + lookAheadSegments);
            }

            sequenceBase += barCount;
            segmentsBeforeStage +=
                (barCount + SessionConstants.BarsPerSegment - 1) /
                SessionConstants.BarsPerSegment;
        }

        return Math.Max(2, segmentsBeforeStage);
    }
        ...
        bool previousTempoUserSet)
    {
        ...
        try
        {
            ...
            var replacement = await IntegratedSessionPlanner.BuildSessionIncrementallyAsync(
                ...
                generatedSegments: liveSegmentLimit);
            if (!IsPlaying || generationVersion != _generationVersion || _audioModule is null) return;
            ...
            await _audioModule.InvokeVoidAsync(
                ...
                duration,
                boundaryBar.StartSeconds);
            var splicedPlan = SplicePlanAtBoundary(
                oldPlan,
                replacement,
                boundaryBar.SequenceIndex,
                boundaryBar.StartSeconds,
                replacementBoundary.StartSeconds);
            _sessionPlan = splicedPlan;
            _positionSeconds = currentPosition;
            if (liveSegmentLimit is not null)
            {
                QueueLiveReplacementExpansion(generationVersion, splicedPlan);
            }
    ...
    private void QueueLiveReplacementExpansion(
        int generationVersion,
        IntegratedSessionPlan preparedPlan)
    {
        var lastPreparedBar = preparedPlan.PlaybackBars
            .OrderByDescending(bar => bar.SequenceIndex)
            .FirstOrDefault();
        if (lastPreparedBar is null) return;

        _ = ExpandLiveReplacementAsync(
            generationVersion,
            lastPreparedBar.SequenceIndex + 1,
            lastPreparedBar.EndSeconds);
    }

    private async Task ExpandLiveReplacementAsync(
        int generationVersion,
        int appendSequenceIndex,
        double appendBoundarySeconds)
    {
        if (_compiledChart is null) return;
        var gateEntered = false;
        try
        {
            // Let the successful short replacement return to the browser first.
            // The full continuation then grows incrementally just like the normal
            // post-Start expansion, rather than delaying the user's control.
            await Task.Delay(1);
            ...
            var expanded = await IntegratedSessionPlanner.BuildSessionIncrementallyAsync(
                ...
                YieldAsync,
                endWithHeadOut: false);
            if (!IsPlaying || generationVersion != _generationVersion || _audioModule is null)
                return;

            var expandedBoundary = expanded.PlaybackBars
                .FirstOrDefault(bar => bar.SequenceIndex == appendSequenceIndex);
            if (expandedBoundary is null)
            {
                // The short plan already reaches the natural end of the full
                // open-ended plan; there is nothing left to append.
                return;
            }
            ...
            var currentPlan = _sessionPlan;
            if (currentPlan is null) return;
            var lastPreparedBar = currentPlan.PlaybackBars
                .OrderByDescending(bar => bar.SequenceIndex)
                .FirstOrDefault();
            if (lastPreparedBar is null ||
                lastPreparedBar.SequenceIndex + 1 != appendSequenceIndex ||
                Math.Abs(lastPreparedBar.EndSeconds - appendBoundarySeconds) > 0.02d)
            {
                // Another live mutation has already replaced or extended this
                // plan. Its generationVersion owns the continuation now.
                return;
            }

            var delta = appendBoundarySeconds - expandedBoundary.StartSeconds;
            var continuation = expanded.Notes
                .Where(note => note.StartSeconds >= expandedBoundary.StartSeconds - 0.001d)
                .Select(note => note with { StartSeconds = note.StartSeconds + delta })
                .ToArray();
            var duration = appendBoundarySeconds +
                Math.Max(0d, expanded.DurationSeconds - expandedBoundary.StartSeconds);
            ...
            _sessionPlan = SplicePlanAtBoundary(
                ...
                expanded,
                appendSequenceIndex,
                appendBoundarySeconds,
                expandedBoundary.StartSeconds);
        }
        catch (OperationCanceledException) { }
        catch (Exception exception)
        {
            SetPlaybackError($"Playback expansion failed: {exception.Message}");
            StatusText =
                $"Playback continues with the prepared section; continuation expansion failed: {exception.Message}";
            await InvokeAsync(StateHasChanged);
        }
        finally
        {
    ...
    private async Task<IJSObjectReference> EnsureAudioModuleAsync() =>
        _audioModule ??= await JS.InvokeAsync<IJSObjectReference>("import", "./js/jampanion-audio.js?v=37");
