...
public class HomeLogic : ComponentBase, IAsyncDisposable
{
    ...
    private async Task<bool> TryConnectChartAsync(bool reportFailure)
    {
        try
        {
            ...
            _chartModule ??= await JS.InvokeAsync<IJSObjectReference>("import", "./js/jazz-chart-host.js?v=48");
    ...
    private async Task<IJSObjectReference> EnsureAudioModuleAsync() =>
            _audioModule ??= await JS.InvokeAsync<IJSObjectReference>("import", "./js/jampanion-audio.js?v=48");
